...
const COUNTER_NUMBERS = Array.from({ length: 100 }, (_, i) => i + 1);
const COUNTER_OPTIONS_HTML = COUNTER_NUMBERS.map((n) => `<option value="${n}">${n}号</option>`).join('');
...
function getCounterOptions(selected) {
  return COUNTER_OPTIONS_HTML.replace(`value="${selected}"`, `value="${selected}" selected`);
}
...
function speak(record) {
  ...
  let spoken = 0;
  const play = () => {
    if (ticket !== speechTicket || spoken >= repeat) return;
...
function renderHeader() {
  return `<div class="card row top-header">
    <div>
      <div class="school-title">Enrollment Service Console</div>
...
function renderHall() {
  ...
  return `<div class="card hall-panel">
    ...
    ${latest ? `<div class="big-number">${latest.number}</div><h3>请到 ${latest.counterNumber} 号登记处</h3>` : '<p class="hall-tip">暂无叫号记录</p>'}
  </div>
  <div class="card">
    <h3>最近叫号记录</h3>
...
function renderDesk() {
  ...
  return `<div class="card">
    ...
    <div class="row">
      <label>登记处编号：
        <select id="counter">${getCounterOptions(deskCounter)}</select>
      </label>
...
function counterRowsHtml() {
  ...
  return rows.map(([counter, teachers], idx) => {
    ...
    return `<div class="row counter-row" data-row="${idx}">
      <label>登记处
        <select class="counter-select">${getCounterOptions(counter)}</select>
      </label>
...
function bindActions() {
  ...
  if (me.role === 'admin') {
    ...
    document.getElementById('addCounterRow').onclick = () => {
      ...
      row.innerHTML = `
      <label>登记处
        <select class="counter-select">${COUNTER_OPTIONS_HTML}</select>
      </label>
      ...
      try {
        ...
        rowsContainer.querySelectorAll('.counter-row').forEach((row) => {
          ...
          const teachers = rawTeachers
            ? rawTeachers.split(/[、,，]/).map((s) => s.trim()).filter(Boolean).slice(0, 3)
            : [];
